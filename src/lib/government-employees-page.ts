...
async function fetchRows(): Promise<RawRow[]> {
  ...
  try {
    ...
    console.warn("[gov-page] failed querying GovernmentEmployeesSection, falling back to GovernmentEmployeesPage", err);
  }
...
export async function getGovernmentEmployeesContent(langValue: string | null): Promise<GovernmentEmployeesPayload> {
  const lang = toLanguage(langValue) ?? "en";
  const rows = await fetchRows();

  const payload: GovernmentEmployeesPayload = {};
  type GovernmentEmployeesSectionValue = GovernmentEmployeesPayload[keyof GovernmentEmployeesPayload];
