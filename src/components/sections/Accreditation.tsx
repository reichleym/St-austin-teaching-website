export default function Accreditation({ blockContent, title, description, className, classNameCard }: { blockContent: { cardTitle: string; cardDescription: string; icon: React.ReactNode; }[]; title: string; className?: string; classNameCard?: string; description?: string }) {
    return (
            ...
            <div className="container">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-[24px]">
                    {blockContent.map((block, index) =>
                        <div className={`flex flex-col rounded-[8px] ${classNameCard || ''}`} key={index}>
